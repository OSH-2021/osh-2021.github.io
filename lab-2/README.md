...
目前的示例程序非常脆弱，无法处理不良的输入（如 `cd ~` 可以运行，但 `cd` 就不行），也不检查各种可能的错误（`chdir`、`fork`、`exec`等系统调用都可能出错）。建议你将它改得更健壮，以方便之后的进一步开发和调试。
...
在使用 shell 的时候输入 `exit` 即可退出 shell，Ctrl-D 等同于 `exit`。
